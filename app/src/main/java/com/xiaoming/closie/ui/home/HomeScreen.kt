...
fun HomeScreen(repo: WardrobeRepository, go: (String) -> Unit) {
    ...
    LazyColumn(
        ...
        item {
            Card(colors = CardDefaults.cardColors(containerColor = androidx.compose.ui.graphics.Color(0xFFF3E5E7))) {
                Column(Modifier.padding(18.dp)) {
                    Text("小柿的今日观察", style = MaterialTheme.typography.titleMedium)
                    Text(observation(repo, owned.size))
                }
            }
        }
        item {
...
private fun observation(repo: WardrobeRepository, count: Int) = when {
    count == 0 -> "我们先从第一件衣服开始吧，我会记得它的故事。"
    repo.listItems().any { repo.wearCount(it.id) == 0 } ->
        "衣橱里有 ${repo.listItems().count { repo.wearCount(it.id) == 0 }} 件还没有穿过，它们正等一个合适的日子。"
    else -> "你已经认真记录了 $count 件衣服。我们慢慢找出你真正最常穿、最自在的样子。"
}

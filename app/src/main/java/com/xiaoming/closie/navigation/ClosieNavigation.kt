...
sealed class Destination(val route:String){data object Home:Destination("home");data object Owned:Destination("owned");data object Returned:Destination("returned");data object Ootd:Destination("ootd");data object Add:Destination("add/{status}");data object Detail:Destination("detail/{id}");data object Edit:Destination("edit/{id}")}
@Composable fun ClosieNavHost(repository:WardrobeRepository){val nav=rememberNavController();NavHost(navController=nav,startDestination=Destination.Home.route){composable(Destination.Home.route){HomeScreen(repository,{nav.navigate(it)})};composable(Destination.Ootd.route){OotdScreen(repository,{nav.popBackStack()})};composable(Destination.Owned.route){ClosetScreen(repository,com.xiaoming.closie.data.model.ItemStatus.OWNED,{nav.navigate(Destination.Detail.route.replace("{id}",it))},{nav.navigate(Destination.Add.route.replace("{status}","OWNED"))},{nav.popBackStack()})};composable(Destination.Returned.route){ClosetScreen(repository,com.xiaoming.closie.data.model.ItemStatus.RETURNED,{nav.navigate(Destination.Detail.route.replace("{id}",it))},{nav.navigate(Destination.Add.route.replace("{status}","RETURNED"))},{nav.popBackStack()})};composable(Destination.Add.route,arguments=listOf(navArgument("status"){type=NavType.StringType})){entry->EditorScreen(repository,null,entry.arguments?.getString("status")?:"OWNED",{nav.popBackStack()})};composable(Destination.Detail.route,arguments=listOf(navArgument("id"){type=NavType.StringType})){entry->DetailScreen(repository,entry.arguments?.getString("id").orEmpty(),{nav.navigate(Destination.Edit.route.replace("{id}",it))},{nav.popBackStack()})};composable(Destination.Edit.route,arguments=listOf(navArgument("id"){type=NavType.StringType})){entry->EditorScreen(repository,entry.arguments?.getString("id"),null,{nav.popBackStack()})}}}
